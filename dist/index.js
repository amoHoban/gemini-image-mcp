...
const OUTPUT_DIR = process.env.GEMINI_IMAGE_OUTPUT_DIR ??
...
function client() {
    ...
    if (!apiKey) {
        throw new Error('No Gemini API key configured. Set GEMINI_API_KEY in your environment, or call `configure_gemini_token`.');
    }
...
async function writeImageFile(data, mimeType, prompt) {
...
function extractImagePart(response) {
    ...
    throw new Error('Model returned no image. The prompt may have triggered a safety block, or the model ID may not support image output.');
}
...
async function edit(sourcePath, prompt) {
...
server.registerTool('generate_image', {
    description: 'Generate a NEW image from a text prompt. Use this ONLY when creating a completely new image, not when modifying an existing one.',
    inputSchema: {
        prompt: z
            .string()
            .min(1)
...
server.registerTool('edit_image', {
    description: 'Edit an existing image on disk. Provide the source path and a prompt describing the change.',
    inputSchema: {
        source_path: z
            .string()
            .min(1)
...
server.registerTool('continue_editing', {
    description: 'Apply another edit on top of the most recent image produced by this MCP session.',
    inputSchema: {
        prompt: z
            .string()
            .min(1)
    ...
    if (!lastImage) {
        throw new Error('No previous image in this session. Call generate_image or edit_image first.');
    }
...
server.registerTool('get_last_image_info', {
...
server.registerTool('configure_gemini_token', {
    description: 'Set the Gemini API key for this MCP session. Overrides GEMINI_API_KEY from the environment until the server restarts.',
    inputSchema: {
...
async function main() {
    ...
    console.error(`gemini-image MCP ready — model=${DEFAULT_MODEL}, outputDir=${OUTPUT_DIR}`);
}
